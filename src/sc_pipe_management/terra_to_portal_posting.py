...
def single_post_to_portal(igvf_data_payload: dict, igvf_utils_api, upload_file: bool = False, patch_existing_post: bool = False) -> str:
    ...
    if (existing_object_id is not None) and (not patch_existing_post):
        raise PortalConflictError(
            f'{igvf_data_payload.get("_profile")} POST failed due to conflict with an existing object: {existing_object_id}.')
    # If conflict exists and patching is expected, return the existing object ID
    if (existing_object_id is not None) and (patch_existing_post):
...
def post_single_document_to_portal(terra_data_record: pd.Series, lab: str, award: str, config_file_path: str, doc_aliases: list, igvf_utils_api, patch_existing_post: bool = False) -> list[PostResult]:
    ...
    Args:
        ...
        doc_aliases (list): Document aliases
        patch_existing_post (bool, optional): Whether to patch an existing post. Defaults to False. See `single_post_to_portal` for more details.
...
def post_single_qc_metric_to_portal(terra_data_record: pd.Series, qc_data_info: dict, qc_of_file_accs: list, lab: str, award: str, analysis_step_version: str, qc_prefix: str, qc_obj_aliases: list, igvf_utils_api, output_root_dir: str = '/igvf/data/', patch_existing_post: bool = False) -> list[PostResult]:
...
def post_single_matrix_file(terra_data_record: pd.Series, col_header: str, curr_file_format: str, curr_derived_from: list, curr_description: str, lab: str, award: str, curr_content_type: str, curr_file_specification: list, igvf_utils_api, upload_file: bool, patch_existing_post: bool = False) -> PostResult:
    ...
    try:
        ...
        curr_post_res = single_post_to_portal(
            igvf_data_payload=curr_mtx_file_payload, igvf_utils_api=igvf_utils_api, upload_file=False, patch_existing_post=patch_existing_post)
        # Upload files
        if upload_file:
...
def post_all_rna_data_to_portal(terra_data_record: pd.Series, lab: str, award: str, igvf_utils_api, upload_file: bool, output_root_dir: str = '/igvf/data/', patch_existing_post: bool = False) -> list[PostResult]:
...
def post_single_alignment_file(terra_data_record: pd.Series, col_header: str, curr_file_format: str, curr_description: str, lab: str, award: str, curr_ctrl_access: bool, curr_derived_from: list, igvf_utils_api, upload_file: bool, patch_existing_post: bool = False) -> PostResult:
    ...
    try:
        ...
        # Post
        curr_post_res = single_post_to_portal(
            igvf_data_payload=curr_alignment_payload, igvf_utils_api=igvf_utils_api, upload_file=False, patch_existing_post=patch_existing_post)
        if upload_file:
            igvf_utils_api.upload_file(
                file_id=curr_post_res, file_path=curr_gs_cloud_link)
        return PostResult.Success(col_header=col_header, accession=curr_post_res)
...
def post_single_tabular_file(terra_data_record: pd.Series, col_header: str, curr_file_format: str, curr_description: str, lab: str, award: str, curr_assembly: str, curr_ctrl_access: bool, curr_derived_from: list, curr_file_specification: list, igvf_utils_api, upload_file: bool, patch_existing_post: bool = False) -> PostResult:
    ...
    try:
        ...
        curr_post_res = single_post_to_portal(
            igvf_data_payload=curr_tab_file_payload, igvf_utils_api=igvf_utils_api, upload_file=False, patch_existing_post=patch_existing_post)
        if upload_file:
            igvf_utils_api.upload_file(
                file_id=curr_post_res, file_path=curr_gs_cloud_link)
        return PostResult.Success(col_header=col_header, accession=curr_post_res)
...
def post_single_index_file(terra_data_record: pd.Series, col_header: str, curr_file_format: str, curr_description: str, lab: str, award: str, curr_ctrl_access: bool, curr_derived_from: list, igvf_utils_api, upload_file: bool, patch_existing_post: bool = False) -> PostResult:
    ...
    try:
        ...
        logging.info(f'Posting {curr_file_aliases} to the portal.')
        curr_post_res = single_post_to_portal(
            igvf_data_payload=curr_index_file_payload, igvf_utils_api=igvf_utils_api, upload_file=False, patch_existing_post=patch_existing_post)
        if upload_file:
            igvf_utils_api.upload_file(
                file_id=curr_post_res, file_path=curr_gs_cloud_link)
        return PostResult.Success(col_header=col_header, accession=curr_post_res)
...
def post_all_atac_alignment_data_to_portal(terra_data_record: pd.Series, lab: str, award: str, curr_ctrl_access: bool, curr_derived_from: list, igvf_utils_api, upload_file: bool, output_root_dir: str = '/igvf/data/', patch_existing_post: bool = False) -> list[PostResult]:
...
def post_all_atac_fragment_data_to_portal(terra_data_record: pd.Series, lab: str, award: str, curr_assembly: str, curr_derived_from: list, igvf_utils_api, upload_file: bool, output_root_dir: str = '/igvf/data/', patch_existing_post: bool = False) -> list[PostResult]:
...
def post_all_atac_data_to_portal(terra_data_record: pd.Series, lab: str, award: str, igvf_api, igvf_utils_api, upload_file: bool, output_root_dir: str = '/igvf/data/', patch_existing_post: bool = False) -> list[PostResult]:
...
def post_all_data_from_one_run(terra_data_record: pd.Series, igvf_api, igvf_utils_api, upload_file: bool, config_file_path: str, doc_aliases: list, pipeline_data_lab: str = POSTING_LAB, pipeline_data_award: str = POSTING_AWARD, output_root_dir: str = '/igvf/data/', patch_existing_post: bool = False) -> RunResult:
...
def post_all_successful_runs(full_terra_data_table: pd.DataFrame, igvf_api, igvf_utils_api, upload_file: bool, config_file_collection: dict, output_root_dir: str = '/igvf/data/', patch_existing_post: bool = False) -> list[PostResult]:
