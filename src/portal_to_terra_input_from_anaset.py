...
# 2) Add download files from the portal to the func (all for seqspec tools)
# 3) Add taxa corresponding genome ref files (how to tell mm39 and mm39_cast?)
...
ASSAY_NAMES_CONVERSION_REF = {
    '/assay-terms/OBI_0002762/': 'atac',    # single-cell ATAC-seq
...
def get_measet_ids_and_other_info_from_analysis_set_acc(analysis_set_accessions: list, igvf_api) -> tuple:
    ...
    """
    analysis_set_obj = igvf_api.analysis_sets(
        accession=analysis_set_accessions).graph[0]
    measurement_ids = [
        measet_id for measet_id in analysis_set_obj.input_file_sets if 'measurement-sets' in measet_id]
    # NOTE: Analysis sets will have multiple taxa
    sample_taxa = list(set([get_samp_taxa_from_samp_id(
        sample_id=samp_id, igvf_api=igvf_api) for samp_id in analysis_set_obj.samples]))
...
def download_file_via_https(igvf_portal_href_url: str) -> str:
    ...
    if response.status_code == 200:
        curr_downloaded_file = tempfile.NamedTemporaryFile()
        with open(curr_downloaded_file.name, 'wb') as file:
            file.write(response.content)
        return curr_downloaded_file.name
    else:
...
class SingleCellInputBuilder:
    def __init__(self, analysis_set_acc: str, igvf_api):
        ...
        self.samp_taxa, self.measet_ids = get_measet_ids_and_other_info_from_analysis_set_acc(
            analysis_set_accessions=[self.analysis_set_acc], igvf_api=self.igvf_api)
        ...
        for column in ['rna_seqspec_urls', 'atac_seqspec_urls']:
            ...
            if curr_seqspec_urls:
                ...
                curr_seqspec_onlist_output = seqspec_onlist_safetychk_and_get(
                    seqspec_file_paths=curr_seqspec_urls, assay_type=curr_assay_type, final_inclusion_list_path=curr_inclusion_list_path)
                ...
                curr_seqspec_index_output = seqspec_index_safetychk_and_get(
                    seqspec_file_paths=curr_seqspec_urls, assay_type=curr_assay_type)
...
def main(query_analysis_set_accs: list, igvf_api, save_to_file: bool = False, output_path='./') -> pd.DataFrame:
    ...
    # Output TSV if needed for local testing
    if save_to_file:
        curr_datetime = datetime.now().strftime('%Y-%m-%d-%H-%M-%S')
        pipeline_input_table.to_csv(os.path.join(
            output_path, f'single-cell_uniform_pipeline_input_table_{curr_datetime}.tsv'), sep='\t')
    else:
